...
if ((which keychain).path | length) > 0 {
  keychain --eval --agents ssh id_rsa
  keychain --eval --agents ssh id_ecdsa
}

let-env _direnv_hook_enabled = 1
def-env direnv-freeze [] {
  $"direnv: disabling shell hook; use direnv-thaw to enable again"
  let-env _direnv_hook_enabled = 0
}
