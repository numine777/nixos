...
local icons = require("numine.icons").kind
...
M.get_filename = function()
	...
	local f = require("numine.utils.functions")
	...
	if not f.isempty(filename) then
		...
		if devicons_ok then
			...
			if f.isempty(file_icon) then
				file_icon = require("numine.icons.kind.File")
			end
		else
			file_icon = ""
			hl_group = "Normal"
...
local get_gps = function()
	...
	if not require("numine.utils.functions").isempty(gps_location) then
	...
	if vim.fn.has("nvim-0.8") == 1 then
		vim.api.nvim_create_autocmd({
			...
			callback = function()
				...
				if not status_ok then
					-- TODO:
					require("numine.configs.navic").get_winbar()
				end
			end,
		})
	end
